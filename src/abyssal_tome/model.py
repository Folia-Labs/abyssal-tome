...
import logging
import uuid
from enum import StrEnum  # Added StrEnum
...
class ProvenanceModel(BaseModel):
    ...
    source_date: str | None = (
        None  # Consider converting to datetime on validation if format is consistent
    )
...
class CardDisplay:
    ...
    def __init__(self, code: str, name: str, rulings: list[RulingModel]) -> None:
        """
...
def load_card_data(file_path_str: str = "../assets/player_cards.json") -> None:  # Added _str suffix
    """
    ...
    try:
        ...
        for ruling_dict in rulings_list_raw:
            try:
                ...
                logging.error(
                    f"Error validating ruling data for ID {ruling_dict.get('id', 'N/A')}: {e}\nData: {ruling_dict}"
                )  # Added N/A default
        ...
        logging.error(
            f"An unexpected error occurred loading rulings: {e}", exc_info=True
        )  # Added exc_info
    ...
    try:
        ...
        for opinion_dict in opinions_list_raw:
            try:
                ...
                if (
                    opinion_obj.applies_to_ruling_id not in temp_opinions_map
                ):  # Corrected applies_to_rulin_id
                    temp_opinions_map[opinion_obj.applies_to_ruling_id] = []
                temp_opinions_map[opinion_obj.applies_to_ruling_id].append(opinion_obj)
                loaded_count += 1
            except Exception as e:
                logging.error(
                    f"Error validating opinionated ruling data for ID {opinion_dict.get('opinion_id', 'N/A')}: {e}\nData: {opinion_dict}"
                )  # Added N/A default
        logging.info(
            f"Loaded {loaded_count} opinionated rulings from {file_path}, mapping to {len(temp_opinions_map)} official ruling IDs."
        )

        for ruling_id, opinions_for_ruling in temp_opinions_map.items():
            if ruling_id in ALL_RULINGS_DATA:
                sorted_opinions = sorted(
                    opinions_for_ruling, key=lambda o: o.provenance.author or ""
                )  # Added or ""
                ALL_RULINGS_DATA[ruling_id].opinions.extend(sorted_opinions)
            else:
                logging.warning(
                    f"Opinionated ruling found for non-existent official ruling ID: {ruling_id}"
                )
        ...
        logging.error(
            f"An unexpected error occurred loading opinionated rulings: {e}", exc_info=True
        )  # Added exc_info
...
def get_rulings_for_card(card_code: str) -> list[RulingModel]:
    ...
    return sorted(
        relevant_rulings.values(),
        key=lambda r_sort: (r_sort.provenance.source_date or "0", r_sort.id),
    )
...
def load_card_info_data(file_path_str: str = "../assets/cards_db.json") -> None:  # Example path
    """
