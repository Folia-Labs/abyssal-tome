...
# Mock functions
def on_card_click(event, page, card_id):
    pass
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("HP-3", "Normal text with [Link](http://example.com/card2) and {italic}Italic Text", 3, 0, 0),
        ...
        ("ER-2", "[Link](http://example.com/) and {unsupported}Unsupported tag", 2, 1, 0),  # Mixed content
    ],
)
def test_replace_special_tags(
    ...
    # Assert
    assert len(spans) == expected_spans_length, f"Test ID {test_id}: Unexpected number of spans created."
    assert len([record for record in caplog.records if record.levelname == 'WARNING']) == expected_warnings, f"Test ID {test_id}: Unexpected number of warnings."
    assert len([record for record in caplog.records if record.levelname == 'ERROR']) == expected_errors, f"Test ID {test_id}: Unexpected number of errors."
